...
unsigned char Inv_S_BOX[256] = {//Inv_S-BOX
...
unsigned char CipherText[4 * Nb] = { 0x69, 0xc4,0xe0, 0xd8, 0x6a, 0x7b, 0x04, 0x30, 0xd8, 0xcd, 0xb7, 0x80, 0x70, 0xb4, 0xc5, 0x5a };
unsigned char Key[4 * Nk] = { 0x00, 0x01,0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f };
unsigned char PlainText[4 * Nb] = { 0, };
word Rk[Nb*(Nr + 1)] = { 0, };

void MakeState(unsigned char State[4][4]) {
	int i, j;

	for (i = 0; i < 4; i++) {
		for (j = 0; j < 4; j++) {
			State[j][i] = CipherText[j + i * 4];
		}
	}
}
void MakeText(unsigned char State[4][4]) {
	int i, j;
	for (i = 0; i < 4; i++) {
		for (j = 0; j < 4; j++) {
			PlainText[j + 4 * i] = State[j][i];
		}
	}
}
void SubWord(word *temp) {
	...
	*temp = (S_BOX[te[0]] << 24) ^ (S_BOX[te[1]] << 16) ^ (S_BOX[te[2]] << 8) ^ S_BOX[te[3]];
}
...
void AddRoundKey(unsigned char State[4][4], int k) {
	unsigned char te[4];
	int i;
	for (i = 0; i < 4; i++) {
		te[0] = Rk[i + 4 * k] >> 24;
		te[1] = (Rk[i + 4 * k] >> 16) & 0xff;
		te[2] = (Rk[i + 4 * k] >> 8) & 0xff;
		te[3] = (Rk[i + 4 * k]) & 0xff;

		State[0][i] = State[0][i] ^ te[0];
		State[1][i] = State[1][i] ^ te[1];
		State[2][i] = State[2][i] ^ te[2];
		State[3][i] = State[3][i] ^ te[3];
	}
}
...
void InvMixColumnFAST(unsigned char State[4][4]) {
	word temp[4];
	int i;

	for (i = 0; i < 4; i++) {
		temp[i] = Table0_dec[State[0][i]] ^ Table1_dec[State[1][i]] ^ Table2_dec[State[2][i]] ^ Table3_dec[State[3][i]];
		State[0][i] = temp[i] >> 24;
		State[1][i] = (temp[i] >> 16) & 0xff;
		State[2][i] = (temp[i] >> 8) & 0xff;
		State[3][i] = temp[i] & 0xff;
	}
}
...
void ChangeRoundKey(int round) {
	int i;
	unsigned char te[4];

	for (i = 0; i < 4; i++) {
		te[0] = Rk[i + 4 * round] >> 24;
		te[1] = (Rk[i + 4 * round] >> 16) & 0xff;
		te[2] = (Rk[i + 4 * round] >> 8) & 0xff;
		te[3] = (Rk[i + 4 * round]) & 0xff;

		Rk[i + 4 * round] = Table0_dec[S_BOX[te[0]]] ^ Table1_dec[S_BOX[te[1]]] ^ Table2_dec[S_BOX[te[2]]] ^ Table3_dec[S_BOX[te[3]]];
	}
}
int main() {
	int i, round;
	...
	for (round = Nr-1; round > 0; round--) {
		InvShiftRows(State);
		InvMixColumnFAST(State);
		ChangeRoundKey(round);
