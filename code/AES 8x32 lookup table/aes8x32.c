...
unsigned char PlainText[4 * Nb] = { 0x00, 0x11,0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff};
unsigned char Key[4 * Nk] = { 0x00, 0x01,0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f};
unsigned char CipherText[4 * Nb] = { 0, };
word Rk[Nb*(Nr + 1)] = { 0, };
...
void MixColumnFAST(unsigned char State[4][4]) {
	word temp[4];
	int i;

	for (i = 0; i < 4; i++) {
		temp[i] = Table0_enc[State[0][i]] ^ Table1_enc[State[1][i]] ^ Table2_enc[State[2][i]] ^ Table3_enc[State[3][i]];
		State[0][i] = temp[i] >> 24;
